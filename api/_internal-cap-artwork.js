...
async function buildCapArtwork({ logoBuffer, frontColor, sideColor, peakColor, nogginLogoColor, layers, customWord }) {
  ...
  const artwork = {
    ...
    side: await buildSidePanel(sideColor, nogginLogoColor, layers.side.size.width, layers.side.size.height, customWord),
...
async function buildSidePanel(colour, logoColour, width, height, customWord) {
  const canvas = new Jimp({ width, height, color: hexToJimpInt(colour) });
  const sideLogo = customWord
    ? await require('./_internal-word').wordImage(customWord, logoColour, SIDE_REFERENCE.logoWidth, SIDE_REFERENCE.logoHeight)
    : await Jimp.read(SIDE_LOGO_PATH);
  ...
  if (customWord) {
    const scale = Math.min(logoWidth / sideLogo.width, SIDE_REFERENCE.logoHeight * scaleY / sideLogo.height);
    sideLogo.resize({w:Math.max(1,Math.round(sideLogo.width*scale)),h:Math.max(1,Math.round(sideLogo.height*scale))});
  } else sideLogo.resize({ w: logoWidth });
