...
module.exports = async (req, res) => {
  ...
  const expected = process.env.INTERNAL_STUDIO_KEY;
  if (!expected || req.headers['x-internal-key'] !== expected) return res.status(401).json({message:'Enter the internal studio access key.'});
  ...
  try {
    ...
    ({ buildCapArtwork, prepareCustomerLogoBuffer } = require('./_internal-cap-artwork'));
...
async function handlePost(req, res, deps) {
  ...
  let customWord;
  try { customWord = require('./_internal-word').validateWord(body.customWord); }
...
bucketHatLogoUrl = bucketHatBlob.url;
  ...
  let designs;
  try {
    designs = await generateAllDesigns({ customWord, logoUrl, bucketHatLogoUrl, logoBuffer, primaryColor, secondaryColor, capDesigns, sessionId, put, getMockup, render, findSmartObjectByName, findOptionalSmartObjectByName, buildBandImage, buildCapArtwork, PRODUCTS, PRODUCT_VARIATIONS });
  } catch (err) {
...
async function generateAllDesigns({ customWord, logoUrl, bucketHatLogoUrl, logoBuffer, primaryColor, secondaryColor, capDesigns, sessionId, put, getMockup, render, findSmartObjectByName, findOptionalSmartObjectByName, buildBandImage, buildCapArtwork, PRODUCTS, PRODUCT_VARIATIONS }) {
  const designs = [];
  ...
  for (const [productKey, config] of Object.entries(PRODUCTS)) {
    ...
    if (customWord && productKey === 'bucketHat') continue;
...
const hex = usePrimaryNogginLogo
  ...
  : whichColour === 'primary'
    ...
    : whichColour === 'secondary'
      ? secondaryColor
      : whichColour;
        if (customWord && productKey === 'beanie' && layerName === 'NOGGIN LOGO') {
          const layer = mockupData.smart_objects.find(item => item.uuid === so.uuid);
          if (!layer?.size?.width || !layer?.size?.height) throw new Error('Beanie wordmark layer dimensions are unavailable; template needs checking.');
          const word = await require('./_internal-word').wordBuffer(customWord, hex, layer.size.width, layer.size.height);
          const blob = await withTimeout(put(`internal-word/${sessionId}-${variationKey}-${Date.now()}.png`,word,{access:'public',contentType:'image/png'}),EXTERNAL_CALL_TIMEOUT_MS,'Uploading custom word');
          smartObjects.push({uuid:so.uuid,asset:{url:blob.url,fit:'fill'}});
        } else smartObjects.push({ uuid: so.uuid, color: { hex } });
      }
