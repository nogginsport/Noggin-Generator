...
async function render(mockupUuid, smartObjects) {
  ...
  const data = await res.json();
  const renderedUrl =
    data?.data?.print_files?.[0]?.export_path ||
    data?.data?.rendered_image_url;

  if (!renderedUrl) {
    throw new Error(`SudoMock render succeeded but returned no image URL: ${JSON.stringify(data)}`);
  }
